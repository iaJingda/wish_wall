...
fn process_instruction(
    ...
    IntroInstruction::InitUserInput {name, message}  => {
          add_wish_intro(program_id, accounts, name, message)
    }
  }
}

pub fn add_wish_intro(
  program_id: &Pubkey,
  accounts: &[AccountInfo],
  name: String,
  message: String,
) -> ProgramResult {
    msg!("Adding wish intro...");
    msg!("Name: {}", name);
    msg!("Message: {}", message);
    ...
    let mut account_data = try_from_slice_unchecked::<WishInfo>(&user_account.data.borrow()).unwrap();
